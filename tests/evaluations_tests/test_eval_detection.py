...
@testing.parameterize(
    {'minoverlap': 0.4,
     'rec': np.array([0.5, 0.5, 1.]),
     'prec': np.array([1., 0.5, 2. / 3.])},
)
class TestEvalDetectionTwoBboxes(unittest.TestCase):

    def test_eval_detection(self):
        bboxes = [np.array([
            [0., 0., 1., 1.], [0., 0., 2., 3.], [1., 1., 3., 3.]])]
        labels = [np.array([0, 0, 0])]
        confs = [np.array([1., 0.9, 0.8])]
        gt_bboxes = [np.array([[0., 0., 1., 1.], [1., 0., 4., 4.]])]
        gt_labels = [np.array([0, 0])]
        # iou is [1., 0.33, 0.083] and [0.09, 0.33, 0.45]
