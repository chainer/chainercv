...
class TestConv2DNormActiv(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if self.norm_type == 'bn':
            norm_kwargs = {'decay': 0.8}
        elif self.norm_type == 'gn':
            norm_kwargs = {'groups': 1}

        initial_bias = 0
        if self.args_style == 'explicit':
            self.l = Conv2DNormActiv(
                ...
                activ=activ, norm_type=self.norm_type, norm_kwargs=norm_kwargs)
    ...
    def check_forward(self, x_data):
        ...
        if self.norm_type == 'bn':
            self.l.norm.avg_var[:] = 1
            self.l.norm.avg_mean[:] = 0
        ...
        if not self.weight_standardization and self.norm_type == 'bn':
            if self.activ == 'relu':
...
class TestConv2DMultiNodeBNActiv(unittest.TestCase):
    ...
    def setUp(self):
        ...
        initialW = np.array([[0, 0, 0], [0, 1, 0], [0, 0, 0]],
                            dtype=np.float32).reshape((1, 1, 3, 3))
        norm_kwargs = {'decay': 0.8, 'comm': create_communicator('naive')}
        initial_bias = 0
        activ = relu
        self.l = Conv2DNormActiv(
            self.in_channels, self.out_channels, self.ksize, self.stride,
            self.pad, self.dilate, initialW=initialW,
            initial_bias=initial_bias, activ=activ, norm_type='bn',
