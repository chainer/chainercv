...
def print_cnn_info(name, link, shape_before, shape_after, time):
    n_stride = (
        int((shape_before[2] + link.pad[0] * 2 -
             link.ksize) / link.stride[0]) + 1,
        int((shape_before[3] + link.pad[1] * 2 -
             link.ksize) / link.stride[1]) + 1
    )

    cost = n_stride[0] * n_stride[1] * shape_before[1] * \
        link.ksize * link.ksize * link.out_channels

    print(
        '%s(%d x %d, stride=%d, pad=%d) (%d x %d x %d) ->\
 (%d x %d x %d) (cost=%d): %.6f[sec]' %
        (name,
         link.W.shape[2],
         link.W.shape[3],
         link.stride[0],
...
def print_pooling_info(
    ...
    cost = n_stride[0] * n_stride[1] * shape_before[1] * \
        filter_size * filter_size * shape_after[1]
