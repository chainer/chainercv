...
from yolov2.utils import box_iou
from yolov2.utils import convert_darknet_model_to_npz
from yolov2.utils import multi_box_iou
...
class YOLOv2Predictor(Chain):
    ...
    def __call__(self, input_x, t):
        ...
        x, y, w, h, conf, prob = F.split_axis(
            F.reshape(output,
                      (batch_size, self.predictor.n_boxes,
        ...
        w_anchor = Variable(np.broadcast_to(np.reshape(
            np.array(self.anchors, dtype=np.float32)[:, 0],
            (self.predictor.n_boxes, 1, 1, 1)), w.shape[1:]))
        h_anchor = Variable(np.broadcast_to(np.reshape(
        ...
        for batch in range(batch_size):
            ...
            for truth_index in range(n_truth_boxes):
                truth_box_x = Variable(
                    np.broadcast_to(
                        np.array(
                            t[batch][truth_index]["x"],
                            dtype=np.float32),
                        box_x.shape))
                truth_box_y = Variable(
                    np.broadcast_to(
                        np.array(
                            t[batch][truth_index]["y"],
                            dtype=np.float32),
                        box_y.shape))
                truth_box_w = Variable(
                    np.broadcast_to(
                        np.array(
                            t[batch][truth_index]["w"],
                            dtype=np.float32),
                        box_w.shape))
                truth_box_h = Variable(
                    np.broadcast_to(
                        np.array(
                            t[batch][truth_index]["h"],
                            dtype=np.float32),
                        box_h.shape))
                ...
                ious.append(multi_box_iou(
                    ...
                    Box(truth_box_x, truth_box_y,
                        truth_box_w, truth_box_h)).data.get())
            ious = np.array(ious)
        ...
        for batch in range(batch_size):
            for truth_box in t[batch]:
                ...
                truth_n = 0
                best_iou = 0.0
                for index, abs_anchor in enumerate(abs_anchors):
                ...
                tprob[batch, int(truth_box["label"]), truth_n,
                      truth_h, truth_w] = 1
                ...
                full_truth_box = Box(
                    float(
                        ...
                        truth_box["w"]), float(
                        truth_box["h"]))
                dx = x[batch][truth_n][0][truth_h][truth_w].data.get()
                dy = y[batch][truth_n][0][truth_h][truth_w].data.get()
                dw = w[batch][truth_n][0][truth_h][truth_w].data.get()
                dh = h[batch][truth_n][0][truth_h][truth_w].data.get()
                predicted_box = Box((dx + truth_w) / grid_w,
                                    (dy + truth_h) / grid_h,
                                    np.exp(dw) * abs_anchors[truth_n][0],
                                    np.exp(dh) * abs_anchors[truth_n][1])
        ...
        tx.to_gpu(), ty.to_gpu(), tw.to_gpu(), th.to_gpu()
        tconf.to_gpu(), tprob.to_gpu()
            ...
            np.broadcast_to(
                np.reshape(
                    np.array(
                        ...
                        :, 0], (self.predictor.n_boxes, 1, 1, 1)), w.shape))
        h_anchor = Variable(
            np.broadcast_to(
                np.reshape(
                    np.array(
                        self.anchors, dtype=np.float32)[
                        :, 1], (self.predictor.n_boxes, 1, 1, 1)), h.shape))
