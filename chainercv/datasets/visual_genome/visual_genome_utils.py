...
def get_visual_genome():
    ...
    os.mkdir(base_path)
    move_files(get_extract_data(vg_100k_url, data_root, 'VG_100K'), base_path)
    move_files(get_extract_data(vg_100k_2_url, data_root, 'VG_100K_2'),
               base_path)
...
def get_image_data():
    data_root = download.get_dataset_directory(root)
    return get_extract_data(image_data_url, data_root, 'image_data.json')


def get_region_descriptions():
    data_root = download.get_dataset_directory(root)
    return get_extract_data(region_descriptions_url, data_root,
                            'region_description.json')


def get_extract_data(url, data_root, member_path):
