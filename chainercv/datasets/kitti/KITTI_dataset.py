...
class KITTIDataset(GetterDataset):
    ...
    def __init__(self, data_dir='auto', date='', driveNo='', color=True, sync=True, isLeft=True):
        ...
        if data_dir == 'auto':
            if sync == True:
                # download sync data
                data_dir = get_KITTI_Sync_Data(os.path.join('pfnet', 'chainercv', 'KITTI'), date, driveNo)
            else:
                # download nosync data
                data_dir = get_KITTI_NoSync_Data(os.path.join('pfnet', 'chainercv', 'KITTI'), date, driveNo)
