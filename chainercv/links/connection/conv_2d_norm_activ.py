...
class Conv2DNormActiv(chainer.Chain):
    ...
    Args:
        ...
        activ (callable): An activation function. The default value is
            ...
            function).
        norm_type ({'bn', 'gn'})
        norm_channel_groups (int)
        norm_kwargs (dict): Keyword arguments passed to initialize a
            normalization layer
    ...
    def __init__(self, in_channels, out_channels, ksize=None,
                 ...
                 weight_standardization=False, activ=relu,
                 norm_type=None,
                 norm_kwargs={}):
        if norm_type is None:
            raise ValueError('please select norm_type')
        if ksize is None:
        ...
        with self.init_scope():
            if weight_standardization:
                ...
                self.conv = Convolution2D(
                    ...
                    groups=groups)
            if norm_type == 'bn':
                if 'comm' in norm_kwargs:
                    self.norm = MultiNodeBatchNormalization(
                        out_channels, **norm_kwargs)
                else:
                    self.norm = BatchNormalization(out_channels, **norm_kwargs)
            elif norm_type == 'gn':
                norm_kwargs['size'] = out_channels
                self.norm = GroupNormalization(**norm_kwargs)
...
# class Conv2DBNActiv(Conv2DNormActiv):
# 
#     def __init__(self, *args, **kwargs):
#         kwargs['norm_type'] = 'bn'
#         super(Conv2DBNActiv, self).__init__(*args, **kwargs)
